...
面向初学者的解释规则集中在 `references/beginner-learning-guide.md`；因果、反事实和证据边界的深度门槛集中在 `references/deep-exploration-guide.md`。模板应保持以下教学不变量：先给通俗心智模型，再给机制推导和技术证据；术语关联真实代码位置；实现导读说明输入、处理、输出、机制和失败信号；每个重要知识点说明适用/不适用边界；练习给出预期观察并保持只读或可安全回退。不要把模板扩展成通用编程教材。
...
- 事实、推断、验证证据和未来计划分开记录。
- 功能笔记以知识正文为主；测试、构建、命令、日志和过程记录只保留支撑知识结论的最小证据，来源台账不得变成测试报告。
- 项目总览和架构与术语必须是 `<project>` 根下的主线兄弟笔记，并由项目级证据生成；功能分支只能提出稳定增量，不能把局部实现写成项目基线。
- 初学者解释不牺牲技术准确性，并始终关联项目证据。
- expanded 捕获不能停在知识点、文件名或调用链罗列；必须有机制、证据、反事实/失败路径、边界和迁移条件的深度审计。
- 自测与练习验证理解，不要求接触生产数据、凭据或破坏性操作。
- 写入后回读验证。

功能模板的内容筛选不应通过删除验证边界来实现“简洁”。应保留“证明了
什么、没有证明什么”，但删除与读者理解机制无关的测试矩阵、原始日志、
完整命令和工程过程；测试代码只有在解释契约、边界、失败模式或验证方法
时才进入正文。

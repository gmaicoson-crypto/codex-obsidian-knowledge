...
3. Skill 同时建立证据地图、学习地图和深度探究审计。审计要求对每个重要结论追踪“现象/输入 → 中间机制 → 输出/信号”，并补充证据、反事实/失败路径、边界和迁移条件；证据地图将结论分为 `[事实]`、`[推断]`、`[计划]`、`[待确认]`。
4. Skill 生成脱敏、排序后的规范证据清单，计算 `evidence_hash` 和稳定 `capture_id`，并记录源 commit；匹配现有身份且内容无变化时返回 no-op。
5. Skill 按“通俗理解 → 机制推导 → 本项目中的文件/符号/数据流证据 → 反事实与边界”组织解释，展示捕获模式、深度探究审计、拟创建或更新的项目总览、架构术语、功能笔记和来源索引，并选择 `analysis`、`design`、`implemented`、`verified` 或 `blocked` 状态。

...
2. Skill 先依据仓库根目录、项目配置、主要入口、组件和现有项目笔记确定项目边界，建立或刷新项目基线；项目总览和架构与术语始终是 `<project>` 目录下的主线兄弟笔记。
3. Skill 只读收集当前任务、当前项目、变更文件、测试/构建证据和已有目标笔记；收集范围可以完整，但不等于全部写入正文。
4. Skill 将证据分为知识正文、最小验证和来源台账。功能正文优先保留机制、决策、边界、术语、调试方法和可迁移经验；测试、构建、命令、日志和过程记录只有在支撑某个知识结论时才以最小形式出现。
5. Skill 同时建立证据地图、学习地图和深度探究审计。审计要求对每个重要结论追踪“现象/输入 → 中间机制 → 输出/信号”，并补充证据、反事实/失败路径、边界和迁移条件；证据地图将结论分为 `[事实]`、`[推断]`、`[计划]`、`[待确认]`。
6. Skill 生成脱敏、排序后的规范证据清单，计算 `evidence_hash` 和稳定 `capture_id`，并记录源 commit；匹配现有身份且内容无变化时返回 no-op。
7. Skill 按“通俗理解 → 机制推导 → 本项目中的文件/符号/数据流证据 → 反事实与边界”组织解释，展示捕获模式、深度探究审计、拟创建或更新的项目总览、架构术语、功能笔记和来源索引，并选择 `analysis`、`design`、`implemented`、`verified` 或 `blocked` 状态。
8. 用户确认后，Codex 使用 Obsidian MCP 的结构化工具创建目录对应的 Markdown 笔记；更新已有功能时只提交预览中批准的增量。
9. Skill 回读笔记，核对路径、frontmatter 和关键内容，再向用户报告结果。
...
3. 功能层用一个具体输入贯穿入口、核心逻辑、状态/输出、边界和调试路径，再通过解释题与安全练习进行理解检查。验证只承担证据边界职责，不把功能层变成测试报告。

这一模型不降低证据标准。通俗解释必须能回指到真实文件、符号、配置、测试或对话；无法确认的架构、机制或反事实必须标为推断或未验证，并写出最小验证办法。

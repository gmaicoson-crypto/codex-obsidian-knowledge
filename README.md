...
把 Codex 写出的代码变成初级程序员能读懂、能验证、能复用的 Obsidian 学习笔记。项目按照“项目导读 + 架构与术语 + 功能知识导读”的结构沉淀知识，不绑定某个项目、仓库或本机 Vault。

本项目主要面向正在使用 Codex 智能体写代码的初级程序员。它不是简单保存“改了哪些文件”或罗列知识点，而是要求读者能够沿着“现象 → 机制 → 证据 → 反事实/失败 → 迁移条件”解释为什么代码产生当前行为，并通过自测问题与安全的小练习确认自己是否真的理解。
...
2. 提炼学习目标、前置知识、关键术语、架构心智模型和端到端代码路径，并追踪输入到结果的中间机制。
3. 用“通俗理解 + 本项目真实代码证据”两层方式生成待写入内容预览，同时审查反事实、失败路径和证据边界。
...
每个功能目录分别记录目标与状态、知识化实现导读、知识结论的验证边界、可复用知识、后续迭代和证据来源。默认使用 `schema_version: 2`、`capture_mode: expanded`、`audience: beginner-programmer` 和 `detail_level: expanded`。`expanded` 不以篇幅判定，而以深度审计判定：每个重要结论都要有机制、证据、反事实或失败边界和迁移条件。测试、构建、命令、日志和开发过程只保留支撑知识结论的最小证据，不把功能目录写成测试报告。每次沉淀还记录 `note_kind`、`capture_id`、`evidence_hash`、`source_commit` 和 `verified_at`，用于查询、迁移、证据锚定和幂等更新。

项目级 `01-架构与术语.md` 从系统边界、组件地图、主流程三个层次解释架构，并维护与真实代码位置关联的术语表。功能级 `01-实现细节.md` 沿一个具体输入跟读调用链，`03-知识应用总结.md` 则沉淀术语、可迁移知识、常见误解、调试方法和练习。`02-实施效果.md` 只做简短的证据边界说明；原始验证材料按需在 `99-相关对话与文件.md` 中引用。

项目总览和架构与术语始终位于 `<project>` 根目录，是所有功能分支共享的主线基线。首次沉淀或基线缺失时，Skill 会先熟悉项目整体结构，再分别预览项目层和功能层更新；功能问题不能单独定义整个项目的架构或术语。
...
检查预览中的项目、功能、状态、目标路径、学习目标、前置知识、术语、架构心智模型、端到端流程、机制解释、反事实/失败路径、决策权衡、证据边界、深度探究审计、自测练习和不确定项。确认后再说：
